...
- Single TV (temperature-volume) equilibrium calculation
...
# Single test condition (from example2.py)
...
# Equilibrium solve
solver.solve(solution, cea.TV, temperature, volume, weights)
...
print(f"Density, kg/m³  : {density:.6e}")
print(f"Volume, m³/kg   : {volume:.6e}")
...
print(f"dH/dV (analytic)       : {dH_dV_analytic:.6e} kJ·m³/kg²")
print(f"dS/dT (analytic)       : {dS_dT_analytic:.6e} kJ/(kg·K²)")
print(f"dS/dV (analytic)       : {dS_dV_analytic:.6e} kJ·m³/(kg²·K)")
...
fig1.suptitle('Enthalpy Derivative Convergence (TV)', fontsize=14, fontweight='bold')
...
fig2.suptitle('Entropy Derivative Convergence (TV)', fontsize=14, fontweight='bold')
...
axes2[1].loglog(h_values, errors_dS_dV, 'o-', linewidth=2, markersize=4, label='Error')
axes2[1].axvline(h_values[opt_idx_dS_dV], color='r', linestyle='--', alpha=0.7, linewidth=1.5, label=f'Optimal h={h_values[opt_idx_dS_dV]:.1e}')
...
axes2[1].set_title('∂S/∂V', fontsize=12)
...
fig3.suptitle('Species Concentration Derivative Convergence (TV)', fontsize=14, fontweight='bold')
